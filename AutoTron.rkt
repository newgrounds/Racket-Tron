...
;; select-random : Not Empty Listof X -> X
;; Selects a random item from a list.
(define (select-random lst)
  (list-ref lst (random (length lst))))

;; List of possible velocities for a tron
(define POSSIBLE-VELS (list 10+0i -10+0i 0+10i 0-10i))
...
(define-interface tron<%>
  [;; A LoB
   ...
   col
   ;; draw : Scene -> Scene
   ;; draws the tron to the given scene
   draw])
...
(define-class tron%
  ...
  (define (draw scn)
    (foldr (λ (blk scene) (place-image (square TRON-SIZE "solid" (field col))
                                       (real-part blk)
                                       (imag-part blk)
                                       scene))
           scn
...
(define-interface world<%>
  [;; This tron
   ...
   ;; Updates the drawing field of the world with the latest
   ;; latest drawing
   update-drawing
   ;; free? : Complex -> Boolean
   ;; Checks if the given location in the world is possible to
   ;; move into
   free?
   ;; possible-next-vels -> [ListOf Complex]
   ;; Generates a list of velocities that the head of the current
   ;; player can move into safely. Empty if none.
   ])

(define-class world%
  (implements world<%>)
  (fields me other p1? drawn)
  ...
  (define (on-receive gd)
    (let ([request (first gd)]
      ...
      (cond [(equal? 'assign-p1 request)
             (new world% (field me) (field other) true (field drawn))]
            ...
            [(equal? 'game-on request)
             (new world%
                  (if (field p1?)
                      (data->tron (first data))
                      (data->tron (second data)))
                  (if (field p1?)
                      (data->tron (second data))
                      (data->tron (first data)))
                  (field p1?)
  ...
  (define (free? spot)
    (let ([ot-lob (send (field other) lob)]
          [me-lob (send (field me) lob)])
      (and (not (or (contains? spot ot-lob)
                    (contains? spot me-lob)))
           (<= 0 (real-part spot) WIDTH)
           (<= 0 (imag-part spot) HEIGHT))))
  
  (define (possible-next-vels)
    (let ([me-lob (send (field me) lob)])
      (if (or (empty? me-lob) (empty? (rest me-lob)))
          empty
          (letrec ([my-head (first me-lob)])
            (filter (lambda(x) (free? (+ my-head x))) POSSIBLE-VELS)))))
  ...
  (define (on-tick)
    (make-package (send this update-drawing)
                  (let ([next-vels (possible-next-vels)])
                    (if (empty? next-vels)
                        (select-random POSSIBLE-VELS)
                        (select-random next-vels)))))
...
(launch-many-worlds ;; make multiple worlds
 (universe (new universe% 
               ...
               (new server-tron% false empty 'black)))
 (big-bang (new world% (new tron% empty "blue") (new tron% empty "red") false EMPTY))
 (big-bang (new world% (new tron% empty "blue") (new tron% empty "red") false EMPTY)))